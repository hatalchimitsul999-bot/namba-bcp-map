...
import { submitProxyReport } from "./actions";
...
export default function ProxyInputForm({ stores }: ProxyInputFormProps) {
  const [storeId, setStoreId] = useState<string>("");
  const [safety, setSafety] = useState("");
  const [business, setBusiness] = useState("");
  const [proxyMethod, setProxyMethod] = useState("");
  const [memo, setMemo] = useState("");
  const [submittedStoreName, setSubmittedStoreName] = useState<string | null>(null);
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await submitProxyReport({
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl shadow-md p-8 max-w-sm w-full text-center space-y-4">
            ...
            <p className="text-sm text-gray-500">
              {submittedStoreName} の情報を代理で登録しました。
            </p>
            <button
              onClick={resetForm}
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-5 space-y-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
          <div>
            ...
            <select
              ...
              disabled={isPending}
              className={`w-full border border-gray-300 rounded-lg px-3 py-2.5 text-base focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50 ${
            ...
            <div className="grid grid-cols-2 gap-2">
              {safetyOptions.map((o) => (
                <button
                  ...
                  disabled={isPending}
                  className={`py-2.5 rounded-lg border-2 text-sm font-medium transition-all disabled:opacity-50 ${
                    safety === o.value
        ...
        {submitError && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-700">
            <p className="font-bold mb-1">保存に失敗しました</p>
            <p>{submitError}</p>
          </div>
        )}
        ...
        <button
          ...
          disabled={isPending || !storeId || !safety || !business || !proxyMethod}
          className={`w-full font-bold py-4 rounded-xl text-base transition-colors ${
            isPending
              ? "bg-gray-300 text-gray-500 cursor-not-allowed"
              : "bg-orange-500 text-white hover:bg-orange-600 disabled:bg-gray-200 disabled:text-gray-500 disabled:cursor-not-allowed"
          }`}
        >
          {isPending ? "保存中..." : "代理入力を登録する"}
        </button>
      </div>
    </div>
  );
